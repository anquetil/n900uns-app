...
export const MintButton = () => {
   ...
   const correctChain = chain?.id == 5 //goerli, 1 for mainnet
   ...
   if(isConnected && correctChain){
      if(isLoading){
         ...
         return <button
            className={`w-fit text-sm text-gray-800 rounded border border-gray-400 p-2 shadow hover:shadow-md bg-white
                  ease-in-out transition-all active:mt-[2px] active:mb-[-2px]`}
            onClick={() => {console.log('here');write?.();}}
            type='button'
         >
            {`Mint this N900UN (0.000899 ETH)`}
         </button> 
      }
   } else {
